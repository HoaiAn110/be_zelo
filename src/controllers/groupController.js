...
const setCoLeader = async (req, res, next) => {
    try {
        ...
        if (group.coLeader.length >= 2) {
            return res.status(400).json({ success: false, message: "Nhóm đã có đủ 2 nhóm phó" });
        }

        // Kiểm tra xem người dùng đã là nhóm phó chưa
        if (group.coLeader.includes(memberId)) {
            return res.status(400).json({ success: false, message: "Người dùng đã là nhóm phó của nhóm này" });
        }
        ...
        // Trả về phản hồi thành công
        return res.status(200).json({ success: true, message: "Người dùng đã được phân quyền thành nhóm phó" });
    } catch (error) {
        next(error);
    }
};
